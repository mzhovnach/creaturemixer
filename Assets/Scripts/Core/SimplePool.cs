...
public static class SimplePool {
	...
	class Pool {
		...
		public GameObject Spawn(Vector3 pos, Quaternion rot) {
			...
			if(inactive.Count==0) {
				...
				obj.name = prefab.name + " (Pooled "+(nextId++)+")";
		...
		public void Despawn(GameObject obj, float time) {
			...
			LeanTween.delayedCall(time, () =>
			{
                if (obj != null)
                {
                    ...
                    inactive.Push(obj);
                    //Debug.Log("New pool size [" + prefab.name + "] = " + inactive.Count);
                }
			});
		}
	...
	static public void Preload(GameObject prefab, int qty = 1) {
		...
		if (pools[prefab].ObjectsInPool() < qty)
		{
	...
	/// </summary>
	static public void Despawn(GameObject obj, float time = 0) {
